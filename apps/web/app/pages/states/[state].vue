...
// Fetch races for this state
const { data, status } = useFetch(`/api/races/by-state/${stateCode.value}`, {
  key: `state-races-${stateCode.value}`,
})

const stateRaces = computed(() => (data.value as any)?.races || [])
const stateTypes = computed(() => (data.value as any)?.types || [])
